        ...
        Papa.parse(csvText, {
          ...
          complete: (results) => {
            const processedData = results.data.map((row: any) => {
              ...
              let dateValue: Date;
              try {
                dateValue = row.date ? new Date(row.date) : new Date();
          ...
          error: (error: Error) => {
            ...
            setError('Failed to parse sales data');
            setLoading(false);
          }
        });
      } catch (err) {
        console.error('Error loading sales data:', err);
        setError('Failed to load sales data. Please make sure the data file exists.');
        setLoading(false);
      }
    };
