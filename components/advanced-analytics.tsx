        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(userRole === 'manager' ? analytics.topAgents : analytics.servicePerformance).map((item, index) => {
                  const name = userRole === 'manager' ? item.agent : item.service
                  const revenue = item.revenue
                  const deals = item.deals
                  const avgDeal = revenue / deals
                  const maxRevenue = userRole === 'manager' 
                    ? analytics.topAgents[0]?.revenue || 1
                    : analytics.servicePerformance[0]?.revenue || 1
